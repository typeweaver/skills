...
- Forward-test new skills and behavior-sensitive revisions in clean contexts
  ...
  one of three exits before the change is ready: build the harder scenario that
  would separate the arms and run it; keep the instruction and record what it
  buys other than behavior, such as a named step instead of an improvisation or
  an auditable justification, as its stated value; or delete it. Handing the
  choice back untaken is not one of the three.
