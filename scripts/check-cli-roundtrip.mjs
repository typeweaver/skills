...
const temporary = mkdtempSync(join(tmpdir(), "equip-it-roundtrip-"));
...
try {
  ...
  const receipt = join(configHome, "equip-it", "receipt.json");
  ...
  assert(!existsSync(join(configHome, "equip-it")), "installer state remains");
} finally {
